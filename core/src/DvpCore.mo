...
/// docs/DESIGN.md for the ICRC-7 handler).
...
shared (initMsg) persistent actor class DvpCore() = self {
  ...
  // before (frozen, the frozen lifecycle). Captured from the install message; on a state-preserving
  ...
  // ── Additive (S1.2): journal-backed ledgers as the escrow primitive ──────────────────────────
  // A thebes-ledger-core TokenLedger escrows a leg as a two-phase RESERVATION instead of a
  // transfer_from once its journal is active: reserve holds owner's amount+fee for the core,
  // settle POSTS it (funds land in the core, then the payout runs exactly as before), reclaim
  // VOIDS it (the hold is released in full - no refund transfer, no fee lost, nothing ever
  // leaves the owner's account).
  //
  // Gate: the ledger's own activation height. The core asks the ledger `journal_active()` at
  // escrow time, so the DvP consumer flips in the same block as the ledger and can never be in
  // journal mode against an inactive facade; below the ledger's height the path is the
  // unchanged transfer_from / transfer path byte for byte. `journalLedgers` is not a mode
  // switch: it declares which ledgers implement the facade interface at all (a legacy ledger
  // has no `journal_active` method). A declared ledger that cannot answer is an explicit
  // refusal, never a silent fallback.
  //
  // The state machine and the invariants are untouched; only the dispatch helpers branch,
  // which is the leg-agnostic seam DvpTypes.mo names for this purpose. Whether a leg is a
  // reservation is a fact recorded per leg (`legReservations`), so a trade escrowed on the
  // legacy path before a ledger's activation settles or reclaims on the legacy path afterwards.
  let journalLedgers = Map.empty<Principal, ()>();
  // (tradeId|side) -> reservation id, and -> the ICRC block of its post
  let legReservations = Map.empty<Text, Nat>();
  let legPosts = Map.empty<Text, Nat>();
  func legKey(trade : T.Trade, s : Side) : Text { Nat.toText(trade.id) # "|" # (switch (s) { case (#A) "A"; case (#B) "B" }) };
  func hasReservation(trade : T.Trade, s : Side) : Bool { Map.get(legReservations, Text.compare, legKey(trade, s)) != null };
  ...
  // attempt is fresh, and the caller gets an error instead of a phantom escrow.
  func valueField(v : ICRC.Value, key : Text) : ?ICRC.Value {
    switch (v) {
      case (#Map(fields)) { for ((k, x) in fields.vals()) { if (k == key) return ?x }; null };
      case (_) null;
    }
  };
  func valueNat(v : ?ICRC.Value) : ?Nat { switch (v) { case (?#Nat(n)) ?n; case (_) null } };
  func valueOwner(v : ?ICRC.Value) : ?Principal {
    switch (v) { case (?acc) { switch (valueField(acc, "owner")) { case (?#Blob(b)) ?Principal.fromBlob(b); case (_) null } }; case null null }
  };
  /// The ICRC-3 block `idx` is a transfer of `amount` from `owner` to the core.
  func blockIsEscrow(ledger : Principal, idx : Nat, owner : Principal, amount : Nat) : async* Bool {
    try {
      let r = await ledgerOf(ledger).icrc3_get_blocks([{ start = idx; length = 1 }]);
      if (r.blocks.size() != 1 or r.blocks[0].id != idx) return false;
      let b = r.blocks[0].block;
      let tx = switch (valueField(b, "tx")) { case (?t) t; case null return false };
      let amt = valueNat(valueField(tx, "amt"));
      let from = valueOwner(valueField(tx, "from"));
      let to = valueOwner(valueField(tx, "to"));
      switch (amt, from, to) {
        case (?a, ?f, ?t) a == amount and Principal.equal(f, owner) and Principal.equal(t, selfPrincipal);
        case _ false;
      }
    } catch (_e) { false }
  };
  /// The journal reservation `res` is this leg's escrow, or was already posted as one.
  func reservationIsEscrow(ledger : Principal, res : Nat, owner : Principal, amount : Nat) : async* { #reserved; #posted : Nat; #no } {
    try {
      switch (await ledgerOf(ledger).journal_reservation(res)) {
        case (?v) {
          if (v.amount == amount and Principal.equal(v.from.owner, owner) and Principal.equal(v.to.owner, selfPrincipal) and Principal.equal(v.reserver, selfPrincipal)) #reserved else #no
        };
        case null {
          switch (await ledgerOf(ledger).reservation_outcome(res)) {
            case (?#posted(idx)) { if (await* blockIsEscrow(ledger, idx, owner, amount)) #posted(idx) else #no };
            case (_) #no;
          }
        };
      }
  ...
  /// Is this leg's ledger a journal facade with its gate open right now?
  func journalActiveOn(ledger : Principal) : async* Result.Result<Bool, Text> {
    if (not Map.containsKey(journalLedgers, Principal.compare, ledger)) return #ok(false);
    try { #ok(await ledgerOf(ledger).journal_active()) }
    catch (_e) { #err("ledger is declared journal-capable but does not answer journal_active") };
  };
  ...
  func escrowLeg(trade : T.Trade, s : Side, owner : Principal) : async* Result.Result<Nat, Text> {
    ...
    switch (l.kind) {
      case (#icrc1 { amount }) {
        let useJournal = switch (await* journalActiveOn(l.ledger)) { case (#ok(b)) b; case (#err(e)) return #err(e) };
        if (useJournal) {
          // Reserve owner's leg for the core until the funding deadline. A replay with
          // the stored created_at_time returns #Duplicate carrying the reservation id.
          try {
            let r = await ledgerOf(l.ledger).reserve_transfer({
              ...
              memo = null;
              created_at_time = ?cat;
              expires_at = ?trade.deadline;
            });
            switch (r) {
              case (#Ok(res)) { Map.add(legReservations, Text.compare, legKey(trade, s), res); recordEscrow(trade, s, res, amount); return #ok(res) };
              case (#Err(#Duplicate({ duplicate_of }))) {
                switch (await* reservationIsEscrow(l.ledger, duplicate_of, owner, amount)) {
                  case (#reserved) { Map.add(legReservations, Text.compare, legKey(trade, s), duplicate_of); recordEscrow(trade, s, duplicate_of, amount); return #ok(duplicate_of) };
                  case (#posted(idx)) { Map.add(legReservations, Text.compare, legKey(trade, s), duplicate_of); Map.add(legPosts, Text.compare, legKey(trade, s), idx); recordEscrow(trade, s, duplicate_of, amount); return #ok(duplicate_of) };
                  case (#no) { st.escrowCat := null; return #err("ledger answered Duplicate for a reservation that is not this escrow; retry with a fresh created_at_time") };
                }
              };
              case (#Err(#InsufficientAllowance({ allowance }))) return #err("insufficient allowance: " # Nat.toText(allowance));
              case (#Err(#InsufficientFunds({ balance }))) return #err("insufficient funds: " # Nat.toText(balance));
              case (#Err(#TooOld)) return #err("escrow created_at_time too old");
              case (#Err(#GenericError({ message; error_code }))) return #err("reservation refused (" # Nat.toText(error_code) # "): " # message);
              case (#Err(_)) return #err("reservation rejected");
            };
          } catch (_e) { return #err("reservation call trapped/rejected") };
        };
        try {
          ...
          switch (r) {
            ...
            case (#Err(#Duplicate({ duplicate_of }))) {
              if (await* blockIsEscrow(l.ledger, duplicate_of, owner, amount)) { recordEscrow(trade, s, duplicate_of, amount); #ok(duplicate_of) }
              else { st.escrowCat := null; #err("ledger answered Duplicate for a block that is not this escrow; retry with a fresh created_at_time") }
            };
  ...
  func payoutLeg(trade : T.Trade, s : Side, recipient : Principal) : async* Result.Result<Nat, Text> {
    ...
    switch (l.kind) {
      case (#icrc1 _) {
        if (hasReservation(trade, s)) {
          // Post the reservation so the funds land in the core's account; idempotent -
          // a lost reply is resolved by asking the ledger for the reservation's outcome.
          switch (await* ensurePosted(trade, s)) { case (#err(e)) return #err(e); case (#ok(_)) {} };
        };
  ...
  func refundLeg(trade : T.Trade, s : Side, owner : Principal) : async* Result.Result<Nat, Text> {
    ...
    switch (l.kind) {
      case (#icrc1 _) {
        if (hasReservation(trade, s) and Map.get(legPosts, Text.compare, legKey(trade, s)) == null) {
          // Void the reservation: the hold is released in full, nothing moved, no fee.
          let res = switch (Map.get(legReservations, Text.compare, legKey(trade, s))) { case (?r) r; case null return #err("no reservation recorded for this leg") };
          try {
            switch (await ledgerOf(l.ledger).void_transfer(res)) {
              case (#Ok(j)) { recordRefund(trade, s, j, st.escrowedAmount); return #ok(j) };
              case (#Err(_)) {
                switch (await ledgerOf(l.ledger).reservation_outcome(res)) {
                  case (?#voided(j)) { recordRefund(trade, s, j, st.escrowedAmount); return #ok(j) };
                  case (?#posted(idx)) { Map.add(legPosts, Text.compare, legKey(trade, s), idx) }; // funds are in the core: refund by transfer below
                  case null return #err("void rejected and the reservation outcome is unknown");
                };
              };
            };
          } catch (_e) { return #err("void call trapped/rejected") };
        };
  ...
  // Post a leg's reservation (journal mode). Idempotent across lost replies: once posted the
  // ledger answers post_transfer with an error, and reservation_outcome says #posted(block).
  func ensurePosted(trade : T.Trade, s : Side) : async* Result.Result<Nat, Text> {
    let key = legKey(trade, s);
    switch (Map.get(legPosts, Text.compare, key)) { case (?idx) return #ok(idx); case null {} };
    let l = leg(trade, s);
    let res = switch (Map.get(legReservations, Text.compare, key)) { case (?r) r; case null return #err("no reservation recorded for this leg") };
    try {
      switch (await ledgerOf(l.ledger).post_transfer(res)) {
        case (#Ok(idx)) { Map.add(legPosts, Text.compare, key, idx); #ok(idx) };
        case (#Err(_)) {
          switch (await ledgerOf(l.ledger).reservation_outcome(res)) {
            case (?#posted(idx)) { Map.add(legPosts, Text.compare, key, idx); #ok(idx) };
            case (?#voided(_)) #err("reservation was voided (expired) before settlement");
            case null #err("post rejected and the reservation outcome is unknown");
          };
        };
      };
    } catch (_e) { #err("post call trapped/rejected") };
  };

  /// Controller-only: declare that a ledger implements the journal facade interface
  /// (thebes-ledger-core TokenLedger). This is a capability declaration, not a mode
  /// switch: the escrow path follows the ledger's own activation height. The
  /// declaration is checked on the spot by calling the ledger.
  public shared ({ caller }) func declareJournalLedger(ledger : Principal) : async Result.Result<Nat64, Text> {
    requireAuth(caller);
    if (not Principal.equal(caller, installer)) return #err("only the core installer may declare a journal ledger");
    let h = try { await ledgerOf(ledger).journal_activation_height() } catch (_e) { return #err("ledger does not answer journal_activation_height: not a journal facade") };
    Map.add(journalLedgers, Principal.compare, ledger, ());
    List.add(adminLog, "JOURNAL_LEDGER|" # Principal.toText(ledger) # "|activation=" # Nat64.toText(h));
    #ok(h);
  };
  public shared ({ caller }) func undeclareJournalLedger(ledger : Principal) : async Result.Result<Text, Text> {
    requireAuth(caller);
    if (not Principal.equal(caller, installer)) return #err("only the core installer may undeclare a journal ledger");
    ignore Map.delete(journalLedgers, Principal.compare, ledger);
    List.add(adminLog, "JOURNAL_LEDGER_REMOVED|" # Principal.toText(ledger));
    #ok("removed");
  };
  public query func isJournalLedger(ledger : Principal) : async Bool { Map.containsKey(journalLedgers, Principal.compare, ledger) };
  public query func legReservation(tradeId : Nat, side : { #A; #B }) : async ?{ reservation : Nat; posted : ?Nat } {
    let key = Nat.toText(tradeId) # "|" # (switch (side) { case (#A) "A"; case (#B) "B" });
    switch (Map.get(legReservations, Text.compare, key)) {
      case (?r) ?{ reservation = r; posted = Map.get(legPosts, Text.compare, key) };
      case null null;
    }
  };
  ...
  /// MMR) - proving the leg-agnostic guarantee. The maker must `icrc37_approve`
  ...
  // to any frozen lifecycle function. The only behavioural difference from
