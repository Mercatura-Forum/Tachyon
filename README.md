...
**Status: verified on a four-validator throwaway chain running the Thebes node; not deployed to
the production chain; not independently audited.** See `docs/VERIFICATION.md` for what has been
...
core/fixtures/  the flaky ledgers used for failure injection
...
mops install
S=$(mops sources)
moc $S -o build/DvpCore.wasm         core/src/DvpCore.mo
moc $S -o build/Matching.wasm        matching/src/Matching.mo
moc $S -o build/ListingRegistry.wasm listing/src/ListingRegistry.mo
...
- **Read after write.** A query issued immediately after a successful update may be served by a
  validator that has not yet applied it. Clients must read the trade's sequence and retry.
...
- **Not run on the production binary.** The verification ran on the node binary of June 2026 on a
  throwaway chain and on local replicas; a run on the production binary with the production
